...
const ProjectsPage: React.FC = () => {
  ...
  const ViewSwitcher = () => (
    <div className="text-sm text-brand-muted">
      <button onClick={() => setView('list')} className={`hover:text-black transition-colors ${view === 'list' ? 'text-black' : ''}`}>
        (List View)
      </button>
      <span className="mx-1">/</span>
      <button onClick={() => setView('grid')} className={`hover:text-black transition-colors ${view === 'grid' ? 'text-black' : ''}`}>
        (Grid View)
      </button>
    </div>
  );
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:p-8 py-16 md:py-24" onMouseMove={view === 'list' ? handleMouseMove : undefined}>
      <AnimateOnScroll className="flex justify-between items-start mb-12">
        <h1 className="text-5xl sm:text-7xl font-bold tracking-tighter text-left uppercase font-sans text-black">All Projects</h1>
        <ViewSwitcher />
      </AnimateOnScroll>
      
      <AnimateOnScroll as="section">
        <div className="flex flex-wrap gap-x-6 gap-y-2 mb-8 border-b border-gray-200 pb-4">
          ...
          <div className="border-b border-gray-200">
            {filteredProjects.map((project, index) => (
              <AnimateOnScroll key={project.id} stagger={index * 50}>
                ...
                >
                  <div className="flex justify-between items-center py-6 border-t border-gray-200 transition-colors duration-300 group-hover:bg-brand-olive-light">
                    <h3 className="text-xl md:text-3xl font-medium text-black transition-all duration-300 group-hover:pl-4 w-1/2 font-sans">{project.title}</h3>
                    <div className="flex items-center space-x-8 text-sm text-brand-muted uppercase tracking-wider w-1/2 justify-end pr-4">
